...
exports.TOPICS = {
    '0xad05b4d6e93e902856d1a65a7cb9b0d22f5a8e0bf540c2006e88f586ac265cf1': 'Position',
...
function processLogs(state, txLogs, pools, tokenDerion, account) {
            ...
            for (const log of logs) {
                ...
                if (to == account) {
                    ...
                    logs.some((log) => {
                        ...
                        // const recipient = getAddress(hexDataSlice(log.topics[2], 12))
                        // const index = getAddress(hexDataSlice(log.topics[3], 12))
                        const [posId, amount, sqrtPrice, valueR] = utils_1.defaultAbiCoder.decode(['bytes32', 'uint', 'uint', 'uint'], log.data);
                        if (posId != id) {
                            return false;
                        }
