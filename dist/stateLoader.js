...
class StateLoader {
    constructor(profile, url, network) {
                ...
                context: (callsReturnContext) => {
                    for (const ret of callsReturnContext) {
                        ...
                        if (ret.reference == 'metadata') {
                            ...
                            if (!config)
                                continue;
                            const [FETCHER, ORACLE, TOKEN_R, K, MARK, INTEREST_HL, PREMIUM_HL, MATURITY, MATURITY_VEST, MATURITY_RATE, OPEN_RATE] = config.map((v) => (v.type == 'BigNumber' ? ethers_1.BigNumber.from(v.hex) : v));
                            pool.config = {
                                FETCHER,
                                ORACLE,
                                TOKEN_R,
